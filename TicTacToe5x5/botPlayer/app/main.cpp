...
#undef TEST_HEURISTICS


#ifdef TEST_HEURISTICS

    int main() {
        Board board{"o oo           xx      xx"};

        board.display(std::cout);
        std::cout << "X: " << board.evaluate(HeuristicType::MIN_MOVES_TO_WIN) << '\n';
        std::cout << "O: " << board.evaluate(HeuristicType::MIN_MOVES_TO_WIN) << '\n';

        Bot botX{board, 4, HeuristicType::SUCCESS_OR_DEFEAT, true};
        Bot botO{board, 4, HeuristicType::SUCCESS_OR_DEFEAT, false};

        Bot::MoveAndScore resX = botX.minMax(5, true);
        Bot::MoveAndScore resO = botO.minMax(5, false);

        std::cout << "minmax X: " << resX.score << '\n';
        std::cout << "minmax O: " << resO.score << '\n';
    }

#else // TEST_HEURISTICS
    ...
    int main(int argc, char *argv[]) {
        ...
        while(!endGame) {
            ...
            if(msg == 0 || msg == 6) {
                ...
                move = bot.move(AlgorithmType::ALPHA_BETA_PRUNING);
                //move = bot.randomMove();
